...
class Population:
    ...
    def computeCrowding(self):
        ...
        for i in range(len(self.population)):
            for j in range(i+1):
                distanceMatrix[i][j] = self.population[i].distance(
                    self.population[j], normVec)
    ...
    def computeFrontRanks(self):
        ...
        while len(tmpPop) > 0:
            ...
            for ind in tmpTmpPop:
                if ind.frontRank == currentFrontRank:
                    tmpPop.remove(ind)
    ...
    def generatePlots(self, title=None, showPlot=True):
        ...
        if self.population[0].frontRank is None:
            plotOrder = [111, 000]
        else:
            plotOrder = [121, 122]
    ...
    def sort(self):
        # newPop = [ind for ind in self.population if ind.life>0]
        # oldPop = [ind for ind in self.population if ind.life<=0]
        # individuals_to_replace = []
        # for oldInd in oldPop:
        #     for newInd in newPop:
        #         if set(oldInd.path) == set(newInd.path):
        #             individuals_to_replace.append(newInd)
        # # 替換
        # for ind in individuals_to_replace:
        #     if ind in newPop:
        #         newPop.remove(ind)
        #         oldPop.append(ind)
        # for ind in oldPop:
        ...
        uniquePop.sort(
            key=lambda ind: (-ind.objectives["step"], ind.objectives["distanceToGoal"]))
        repetitivePop.sort(
            key=lambda ind: (-ind.objectives["step"], ind.objectives["distanceToGoal"]))
        # oldPop.sort(
        #     key=lambda ind: (-ind.objectives["step"], ind.objectives["distanceToGoal"]))
        uniquePop.extend(repetitivePop)
        # uniquePop.extend(oldPop)
